...
def main():
    ...
    with open('StatsReport.txt', 'w') as f:

        # get this in a function
        currentDateTime = str(datetime.datetime.now())

        # TODO add report name (made it from cmd args)
        f.write("Statistics Report for \n\n")
        f.write("This report was ran on: " + currentDateTime + "\n")
        # TODO get last modified date
        f.write("The json file read was last updated on: \n")
        ...
        def bubbleSortByLength(listBoi):
            ...
            def step1and2func():
                ...
                for key in JsonData:
                    ...
                    f.write("Total Number or Key Value Pairs: " + str(total) + '\n\n\n')
                    f.write("The top 5 longest strings by word count: \n")

                    counter = 0
                    while(counter < 5):
                        f.write("   " + sortedList[counter].key + " : " + str(sortedList[counter].wordCount) + "\n")
                        counter += 1
                    ...
                    for key, val in KeyDupes.items():
                        for ele in consumedData:
                            if key == ele.text:
                                ...
                                f.write("\n\n")
                                f.write("The following string values are duplicated. Underneath the value are the keys it is found in: \n")

                        #This is just printing items that are duplicated (>=2)
                        for ele in finalDict:
                            f.write("   \"" + ele + "\" : " + str(len(finalDict[ele])) + "\n")
                        for ele2 in finalDict[ele]:
                            f.write("      " + ele2 + "\n")

                            f.write("\n")
                            #f.write("The top 5 duplicated string are: ")
            ...
            def step3():
                ...
                for key in JsonData:
                    ...
                    for ele in ArrayOfOccurences:
                        ...
                        print("The key " + obj.key + " appears " +
                              str(grandTotal) + " times")
